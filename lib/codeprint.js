...
codePrint = (function ($) {
    ...
    function getSourceCode($elem, isGrouped) {
        ...
        html = removeLeadingAndTrailingWhitespace(html);

        // Return html having stripped out any leading whitespace
        return html;
    }

    function removeLeadingAndTrailingWhitespace(string) {
        var regexTrailing = /\s+$/g,
            regexLeading = /^\s+/g;

        return string.replace(regexLeading, '').replace(regexTrailing, '');
    }
